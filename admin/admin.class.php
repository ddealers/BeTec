...
class AdminClass extends MYDB{
	...
	public function docs($idu, $nombre, $email, $correo, $tipo){
		$response = false;
		$url = "http://borntobetec.mty.itesm.mx/documentacion.php?s=".$correo;
		...
		if($tipo == 0){
			if(mail($email, 'Completa tu Registro', $mail_FB, $cabeceras)){
				$response = true;
			}
		}elseif($tipo == 1){
			if(mail($email, 'Completa tu Registro', $mail_FA, $cabeceras)){
				$response = true;
			}
		}
		echo $response;
